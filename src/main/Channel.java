...
public class Channel {
	 ...
	 */
	public void toggleState() throws UserErrorException{
		//if on then off
		//if off then on
		if(!state){
			if(getSensor().getType().equals(SensorType.NONE)) throw new UserErrorException("You must connect a sensor before enableing");
		}
		state = !state; 
	}
	/**
	...
	public Time triggerChannel(){
		...
		return new Time(ChronoTimer1009System.globalTime.getTime());
